...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-60 bg-white dark:bg-slate-950/90 border-r border-slate-200 dark:border-slate-800/80 flex flex-col justify-between flex-shrink-0 select-none z-30 transition-all shadow-sm">
      {/* Brand Header */}
      <div>
        <div className="p-3.5 border-b border-slate-200 dark:border-slate-800/80 flex items-center space-x-2.5 bg-slate-50/50 dark:bg-slate-900/40">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center shadow-md text-white font-bold">
            <Network className="w-4 h-4" />
          </div>
          <div>
        ...
        {/* Active Case Badge (White card with silver lining in Light mode) */}
        <div className="p-2.5 mx-3 my-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-cyan-500/20 text-xs shadow-sm">
        ...
        {/* Navigation Section */}
        <nav className="px-3 space-y-3 pt-1">
          {navItems.map((group) => (
            <div key={group.group}>
              <div className="px-2 pb-1 text-[9px] font-mono font-bold text-slate-400 dark:text-slate-500 tracking-wider">
                {group.group}
              </div>
              <div className="space-y-1">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-cyan-600 dark:text-cyan-400' : item.disabled ? 'text-slate-300 dark:text-slate-700' : 'text-slate-500 dark:text-slate-400'}`} />
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800/80 text-[10px] text-slate-500 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-950">
        <div className="flex items-center space-x-1.5">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
          <span className="font-mono font-medium text-slate-700 dark:text-slate-400">Grounding Guardrails Active</span>
        </div>
